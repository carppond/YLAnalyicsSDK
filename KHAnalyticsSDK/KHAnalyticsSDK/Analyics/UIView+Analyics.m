...
- (BOOL)shouldTrackAppViewScreen {
    ...
    dispatch_once(&onceToken, ^{
        // 获取黑名单路径
        NSString *path = [[NSBundle bundleForClass:[KHSensorsAnalyicsSDK class]] pathForResource:@"KHAnalycisBlackList" ofType:@"plist"];
        /// 读取文件中黑名单类名的数组
...
- (NSString *)analyicsItemPath {
    ...
    NSInteger index = [KHSensorsAnalyicsTool itemIndexForResponder:self];
...
- (NSString *)analyicsHeatMapPath {
    ...
    NSString *identifier = [KHSensorsAnalyicsTool viewIdentifierForView:self];
    if (identifier) {
        return identifier;
    }
    return [KHSensorsAnalyicsTool itemHeatMapPathForResponder:self];
}
...
- (NSString *)analyicsSimilarPathWithIndexPath:(NSIndexPath *)indexPath {
    if ([KHSensorsAnalyicsTool isAlertClickForView:self]) {
