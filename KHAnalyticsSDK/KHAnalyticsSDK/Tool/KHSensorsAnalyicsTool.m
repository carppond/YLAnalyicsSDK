...
@implementation KHSensorsAnalyicsTool
...
+ (UIViewController *)findSuperViewControllerByView:(UIView *)view {
    UIViewController *viewController = [KHSensorsAnalyicsTool findNextViewControllerByResponder:view];
    if ([viewController isKindOfClass:UINavigationController.class]) {
        viewController = [KHSensorsAnalyicsTool currentViewController];
    }
...
+ (UIViewController *)currentViewController {
    ...
    void (^ block)(void) = ^{
        ...
        currentViewController = [KHSensorsAnalyicsTool findCurrentViewControllerFromRootViewController:rootViewController isRoot:YES];
    };

    [KHSensorsAnalyicsCommonTool performBlockOnMainThread:block];
...
@implementation KHSensorsAnalyicsTool (ViewPath)
...
+ (BOOL)isIgnoredVisualizedAutoTrackForViewController:(UIViewController *)viewController {
    ...
    KHSensorsAnalyicsSDK *sa = [KHSensorsAnalyicsSDK sharedInstance];
...
+ (NSArray<NSString *> *)viewPathsForViewController:(UIViewController<KHAnalyicsTrackViewPathProperty> *)viewController {
    NSMutableArray *viewPaths = [NSMutableArray array];
    do {
        [viewPaths addObject:viewController.analyicsHeatMapPath];
        viewController = (UIViewController<KHAnalyicsTrackViewPathProperty> *)viewController.parentViewController;
    } while (viewController);

    UIViewController<KHAnalyicsTrackViewPathProperty> *vc = (UIViewController<KHAnalyicsTrackViewPathProperty> *)viewController.presentingViewController;
    if ([vc conformsToProtocol:@protocol(KHAnalyicsTrackViewPathProperty)]) {
...
// 找到 view 的路径数组
+ (NSArray<NSString *> *)viewPathsForView:(UIView<KHAnalyicsTrackViewPathProperty> *)view {
    ...
    if ([view isKindOfClass:UIViewController.class] && [view conformsToProtocol:@protocol(KHAnalyicsTrackViewPathProperty)]) {
        // 遍历 controller 层 路径
        [viewPathArray addObjectsFromArray:[self viewPathsForViewController:(UIViewController<KHAnalyicsTrackViewPathProperty> *)view]];
    }
...
+ (NSString *)viewSimilarPathForView:(UIView *)view atViewController:(UIViewController *)viewController shouldSimilarPath:(BOOL)shouldSimilarPath {
    ...
    if ([view isKindOfClass:UIAlertController.class]) {
        UIViewController<KHAnalyicsTrackViewPathProperty> *viewController = (UIViewController<KHAnalyicsTrackViewPathProperty> *)view;
...
@implementation KHSensorsAnalyicsTool (Validator)
