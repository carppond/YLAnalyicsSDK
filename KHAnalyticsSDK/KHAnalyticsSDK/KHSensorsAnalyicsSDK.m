...
#import "KHSensorsAnalyticsKeychainTool.h"
...
static NSString * const KHSensorsAnalyticsAnonymousId = @"cn.analyics.anonymous_id";
static NSString * const KHSensorsAnalyticsKeychainService = @"cn.sensorsdata.SensorsAnalytics.id";
static NSString * const KHSensorsAnalyticsLoginId = @"cn.sensorsdata.login_id";
static NSString * const KHSensorsAnalyticsEventBeginKey = @"event_begin";
static NSString * const KHSensorsAnalyticsEventDurationKey = @"event_duration";
static NSString * const KHSensorsAnalyticsEventIsPauseKey = @"is_pause";
static NSUInteger const KHSensorsAnalyticsDefalutFlushEventCount = 50;
static NSString * const KHSensorsAnalyticsJavaScriptTrackEventScheme = @"sensorsanalytics:// trackEvent";


@interface KHSensorsAnalyicsSDK ()
/// 预制属性，用户的基本信息
...
/// 发送网络请求的对象
@property (nonatomic, strong) KHSensorsAnalyticsNetwork *network;

@property (nonatomic, strong) dispatch_queue_t serialQueue;
/// 定时上传事件的计时器
@property (nonatomic, strong) NSTimer *flushTimer;
/// 数据同步策略
@property (nonatomic, assign) KHSensorsAnalyicsTacticsType tacticsType;
...
@end
@implementation KHSensorsAnalyicsSDK {
    NSString *_anonymousId;
}
#pragma mark - Cycle life

static KHSensorsAnalyicsSDK *_sharedInstance = nil;
+ (void)startWithServerURL:(NSString *)urlString
          withFlushTactics:(KHSensorsAnalyicsTacticsType)type {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[KHSensorsAnalyicsSDK alloc] initWithServerURL:urlString withFlushTactics:type];
    });
}
...
- (instancetype)initWithServerURL:(NSString *)urlString
    ...
    self = [super init];
    if (self) {
        self.autoMaticPoperties = [KHSystemParam getCollectAutomaticProperties];
        /// 设置是否为被动启动
        ...
        self.loginInfo = [[NSUserDefaults standardUserDefaults] objectForKey:KHSensorsAnalyticsLoginId];
        // TODO: 缺少一个URL
        self.network = [[KHSensorsAnalyticsNetwork alloc] initWithServerURL:[NSURL URLWithString:urlString]];
        ...
        if (type == KHSensorsAnalyicsTacticsType_BuildSize) {
            self.flushBulkSize = 100000;
        } else if (type == KHSensorsAnalyicsTacticsType_TimeInterval) {
            self.flushInterval = 15;
        } else {
            [self startFlushTimer];
        }
        // 调用异常处理单例对象，进行初始化
//        [KHSensorsAnalyticsExceptionHandler sharedInstance];
...
- (void)applicationDidEnterBackground:(NSNotification *)notification {
    ...
    [self.trackTimer enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSDictionary * _Nonnull obj, BOOL * _Nonnull stop) {
        if (![obj[KHSensorsAnalyticsEventIsPauseKey] boolValue]) {
            [self.enterBackgroundTrackTimerEvents addObject:key];
            [self trackTimerPause:key];
        }
    }];
    // 进入后台策略
    if (self.tacticsType == KHSensorsAnalyicsTacticsType_EnterBackground) {
        ...
        dispatch_async(self.serialQueue, ^{
            // 发送数据
            [self flushByEventCount:KHSensorsAnalyticsDefalutFlushEventCount background:YES];
            // 结束后台任务
            endBackgroundTask();
        });
    }
    if (self.tacticsType == KHSensorsAnalyicsTacticsType_TimeInterval) {
        // 停止计时器
        [self stopFlushTimer];
    }
}
...
- (void)login:(NSDictionary<NSString *,id> *)loginInfo {
    ...
    // 在本地保存登录ID
    [[NSUserDefaults standardUserDefaults] setObject:loginInfo forKey:KHSensorsAnalyticsLoginId];
...
- (void)saveAnonymousId:(NSString *)anonymousId {
    // 保存设备ID
    [[NSUserDefaults standardUserDefaults] setObject:anonymousId forKey:KHSensorsAnalyticsAnonymousId];
    [[NSUserDefaults standardUserDefaults] synchronize];
    
    KHSensorsAnalyticsKeychainTool *item = [[KHSensorsAnalyticsKeychainTool alloc] initWithService:KHSensorsAnalyticsKeychainService key:KHSensorsAnalyticsAnonymousId];
    if (anonymousId) {
...
- (NSString *)anonymousId {
    ...
    if (_anonymousId) {
        // 保存设备ID
        [[NSUserDefaults standardUserDefaults] setObject:_anonymousId forKey:KHSensorsAnalyticsAnonymousId];
        [[NSUserDefaults standardUserDefaults] synchronize];
        return _anonymousId;
    }
    // 从NSUserDefaults中读取设备ID
    _anonymousId = [[NSUserDefaults standardUserDefaults] objectForKey:KHSensorsAnalyticsAnonymousId];
    if (_anonymousId) {
        return _anonymousId;
    }
...
- (void)flush {
    dispatch_async(self.serialQueue, ^{
        // 默认一次向服务端发送50条数据
        [self flushByEventCount:KHSensorsAnalyticsDefalutFlushEventCount background:NO];
    });
}
...
@implementation KHSensorsAnalyicsSDK (Track)
...
- (void)track:(NSString *)eventName properties:(NSDictionary<NSString *,id> *)properties {
    ...
    if (self.tacticsType != KHSensorsAnalyicsTacticsType_BuildSize) {
        return;
    }
...
- (void)trackAppClickWithView:(UIView *)view properties:(nullable NSDictionary <NSString *, id> *)properties {
    ...
    [eventProperties addEntriesFromDictionary:properties];
    // 触发AppClick事件
    [[KHSensorsAnalyicsSDK sharedInstance] track:@"AppClick" properties: eventProperties];
}
...
- (void)trackAppWKLoadWithView:(UIView *)view properties:(nullable NSDictionary <NSString *, id> *)properties {
    ...
    [eventProperties addEntriesFromDictionary:properties];
    // 触发AppClick事件
    [[KHSensorsAnalyicsSDK sharedInstance] track:@"AppWKLoad" properties: eventProperties];
}
...
- (void)trackAppClickWithTableView:(UITableView *)tableView
    ...
    [[KHSensorsAnalyicsSDK sharedInstance] trackAppClickWithView:tableView properties:eventProperties];
}
...
- (void)trackAppClickWithCollectionView:(UICollectionView *)collectionView
    ...
    [eventProperties addEntriesFromDictionary:properties];
    // 触发 AppClick 事件
    [[KHSensorsAnalyicsSDK sharedInstance] trackAppClickWithView:collectionView properties:eventProperties];
}
...
- (void)trackAppWithWKWebView:(WKWebView *)webView
    ...
    [eventProperties addEntriesFromDictionary:properties];
    // 触发 AppClick 事件
    [[KHSensorsAnalyicsSDK sharedInstance] trackAppWKLoadWithView:webView properties:eventProperties];
}
...
@implementation KHSensorsAnalyicsSDK (Timer)

- (void)trackTimerStart:(NSString *)event {
    // 记录事件开始时间
    self.trackTimer[event] = @{KHSensorsAnalyticsEventBeginKey: @([KHSensorsAnalyicsSDK systemUpTime])};
}
...
- (void)trackTimerEnd:(NSString *)event properties:(NSDictionary *)properties {
    ...
    if ([eventTimer[KHSensorsAnalyticsEventIsPauseKey] boolValue]) {
        // 获取事件时长
        double eventDuration = [eventTimer[KHSensorsAnalyticsEventDurationKey] doubleValue] / 100;
        p[@"event_duration"] = [NSString stringWithFormat:@"%.3f", eventDuration];
    } else {
        double beginTime = [(NSNumber *)eventTimer[KHSensorsAnalyticsEventBeginKey] doubleValue];
        double currentTime = [KHSensorsAnalyicsSDK systemUpTime];
        // 计算事件市场
        double eventDuration = currentTime - beginTime + [eventTimer[KHSensorsAnalyticsEventDurationKey] doubleValue];
...
- (void)trackTimerPause:(NSString *)event {
    ...
    // 获取当前系统启动时间
    double systemUpTime = [KHSensorsAnalyicsSDK systemUpTime];
    // 获取开始时间
    double beginTime = [eventTimer[KHSensorsAnalyticsEventBeginKey] doubleValue];
    // 计算暂停前统计的时长
    double duration = [eventTimer[KHSensorsAnalyticsEventDurationKey] doubleValue] + systemUpTime - beginTime;
    eventTimer[KHSensorsAnalyticsEventDurationKey] = @(duration);
    // 事件处于暂停状态
    eventTimer[KHSensorsAnalyticsEventIsPauseKey] = @(YES);
...
- (void)trackTimerResume:(NSString *)event {
    ...
    double systemUpTime = [KHSensorsAnalyicsSDK systemUpTime];
    // 重置事件开始时间
    eventTimer[KHSensorsAnalyticsEventBeginKey] = @(systemUpTime);
    // 将事件暂停标记设置为NO
    eventTimer[KHSensorsAnalyticsEventIsPauseKey] = @(NO);
    self.trackTimer[event] = eventTimer;
}
...
@implementation KHSensorsAnalyicsSDK (WebView)
...
- (BOOL)shouldTrackWithWebView:(id)webView request:(NSURLRequest *)request {
    ...
    if ([urlString rangeOfString:KHSensorsAnalyticsJavaScriptTrackEventScheme].location == NSNotFound) {
        return NO;
    }
...
@implementation KHSensorsAnalyicsSDK (VisualizedAutoTrack)
