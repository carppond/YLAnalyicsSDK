...
typedef NS_ENUM(NSInteger, YLSensorsAnalyicsTacticsType) {
    /// 本地缓存数据条数发送策略
    YLSensorsAnalyicsTacticsType_BuildSize          = 1 << 1,
    /// 时间间隔发送策略
    YLSensorsAnalyicsTacticsType_TimeInterval       = 1 << 2,
    /// 进入后台发送策略
    YLSensorsAnalyicsTacticsType_EnterBackground    = 1 << 3
};

@interface YLSensorsAnalyicsSDK : NSObject
...
@interface YLSensorsAnalyicsSDK (Track)
...
@interface YLSensorsAnalyicsSDK (Timer)
...
@interface YLSensorsAnalyicsSDK (WebView)
...
@interface YLSensorsAnalyicsSDK (VisualizedAutoTrack)
