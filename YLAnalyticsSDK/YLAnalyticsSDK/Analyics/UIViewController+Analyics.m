...
static NSString * const kSensorsDataBlackListFileName = @"YLAnalycisBlackList";
...
- (void)analyics_viewDidAppear:(BOOL)animated {
    ...
    if ([self shouldTrackAppViewScreen]) {
        ...
        [[YLSensorsAnalyicsSDK sharedInstance] track:@"AppViewScreen" properties:properties];
    }
}
...
- (BOOL)shouldTrackAppViewScreen {
    ...
    dispatch_once(&onceToken, ^{
        // 获取黑名单路径
        NSString *path = [[NSBundle bundleForClass:[YLSensorsAnalyicsSDK class]] pathForResource:kSensorsDataBlackListFileName ofType:@"plist"];
        /// 读取文件中黑名单类名的数组
