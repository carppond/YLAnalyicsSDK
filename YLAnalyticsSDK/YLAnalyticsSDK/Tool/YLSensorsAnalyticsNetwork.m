...
/// 网络请求结束处理回调类型
typedef void(^YLSAURLSessionTaskCompletionHandler)(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error);


@interface YLSensorsAnalyticsNetwork ()<NSURLSessionDelegate>

@property (nonatomic, strong) NSURLSession *session;

@end

@implementation YLSensorsAnalyticsNetwork
...
- (BOOL)flushEvents:(NSArray<NSString *> *)events {
    ...
    YLSAURLSessionTaskCompletionHandler handler = ^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
